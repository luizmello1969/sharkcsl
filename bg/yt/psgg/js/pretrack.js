var campaignID = "696d2aa1263277b0f168fb66";
...
var initialSrc = "https://atlls.bodypathapp.com/"+campaignID+"?format=json";
...
setTimeout(function(){
	if (!urlParams.get('rtkcid')) {
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState == 4 && xhr.status == 200) {
				...
				document.querySelectorAll('a').forEach(function(el) {
					if (el.href.indexOf("atlls.bodypathapp.com/preclick")>-1) {
				...
				xhrr.open("GET", "https://atlls.bodypathapp.com/preview?clickid="+rawData.clickid)
				xhrr.send();
			}
		}
		...
		xhrTrack.open("GET", "https://atlls.bodypathapp.com/preview?clickid="+rtkClickID )
		xhrTrack.send();
		document.querySelectorAll('a').forEach(function(el) {
			if (el.href.indexOf("atlls.bodypathapp.com/preclick")>-1) {
				if (el.href.indexOf('?')>-1) {
